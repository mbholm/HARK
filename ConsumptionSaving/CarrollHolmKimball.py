...
if __name__ == '__main__':
    import CHKParameters as Params
...
###############################################################################
    ...
    #fig, ax = plt.subplots()    
    plt.plot(PFexample.cFunc[0])#,IndShockExample.cFunc[0])    
    plt.axis([IndShockExample.solution[0].mNrmMin,5, 0, 1])
    #plotFuncs([PFexample.cFunc[0],IndShockExample.cFunc[0]],IndShockExample.solution[0].mNrmMin,5)
    plt.ylabel('c(w), \bar{c}(w)')    
        
    plt.show()    
    plt.savefig('CompareCFuncs.eps', format='eps', dpi=1000)    
    ...
    LifecycleExample.delFromTimeInv('BoroCnstArt')
    LifecycleExample.addToTimeVary('BoroCnstArt')
    LifecycleExample(BoroCnstArt = [None,None,None,None,None,None,None,None,None,None])
    ...
    #plotFuncs(LifecycleExample.cFunc[:LifecycleExample.T_age-1],mMin,5)

    BCNST = IndShockConsumerType(**Params.init_lifecycle)
    BCNST.cycles = 1 # Make this consumer live a sequence of periods exactly once
    BCNST.delFromTimeInv('BoroCnstArt')
    BCNST.addToTimeVary('BoroCnstArt')
    BCNST(BoroCnstArt = [None,0,None,None,None,None,None,None,None,None])
    BCNST.solve()
    BCNST.unpackcFunc()
    BCNST.timeFwd()
    
    # Plot the consumption functions during working life
    print('Consumption functions, perfect foresight vs. borrowing constraint at 0 in period 2')
    mMin = min([BCNST.solution[t].mNrmMin for t in range(BCNST.T_cycle)])
    plotFuncs([BCNST.cFunc[0],LifecycleExample.cFunc[0]],-9,5)
